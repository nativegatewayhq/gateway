# Native Gateway async Go sidecar template

This isolated module implements only the public `plugin-sdk/async/v1` contract.
...
Run the async profile from the Gateway checkout with the same purpose-specific
...
go run ./cmd/gateway-plugin-conformance -profile async-v1 \
  ...
  -plugin-id provider.async-example \
  -endpoints-json '{"async-sidecar":"http://127.0.0.1:8082"}' \
  -auth-secret-env-json '{"async-sidecar-token":"PLUGIN_AUTH_TOKEN"}' \
