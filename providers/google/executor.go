...
func New(credentials *providercredentials.Registry, timeout time.Duration) *Executor {
	...
	return NewWithClient(credentials, timeout, &http.Client{Transport: transport})
}
...
func newExecutor(origin *url.URL, client *http.Client, credentials *providercredentials.Registry, timeout time.Duration) *Executor {
	return &Executor{origin: origin, client: client, credentials: credentials, timeout: timeout}
}
...
func (executor *Executor) GenerateContent(ctx context.Context, input GenerateContentRequest) (*http.Response, error) {
	...
	requestURL.User = nil
	requestURL.Fragment = ""
	requestURL.Path = strings.TrimSuffix(executor.origin.Path, "/") + "/v1beta/models/" + input.Model + ":generateContent"
	requestURL.RawPath = ""
