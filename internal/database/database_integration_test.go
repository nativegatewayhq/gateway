...
func TestMigrateIsRepeatable(t *testing.T) {
	...
	if err := pool.QueryRow(ctx, `SELECT NOT EXISTS (
		SELECT required.name FROM (VALUES ('users'),('organizations'),('organization_memberships'),('projects'),('service_api_keys'),('organization_wallets'),('wallet_reservations'),('wallet_operations'),('ledger_entries'),('provider_channels'),('provider_prices'),('price_publications')) required(name)
