...
func (s *Service) Begin(ctx context.Context, r BeginRequest) (Charge, error) {
	...
	estimate, err := s.estimator.EstimateInTx(ctx, tx, chatpricing.Request{ChannelID: r.ChannelID, Model: r.Model, MaximumInputTokens: r.MaximumInputTokens, MaximumOutputTokens: r.MaximumOutputTokens})
	if err != nil {
		return Charge{}, err
	}
	id, err := s.id("chc_")
	...
	reservation, err := s.wallet.ReserveInTx(ctx, tx, r.OrganizationID, r.ProjectID, "chat:"+operationIdentity, estimate.MaximumSale, "chat-reserve:"+operationIdentity)
	if err != nil {
		return Charge{}, err
	}
	charge := Charge{ID: id, RequestID: r.RequestID, OrganizationID: r.OrganizationID, ProjectID: r.ProjectID, APIKeyID: r.APIKeyID, Model: r.Model, ChannelID: r.ChannelID, PriceID: estimate.Price.ID, Currency: estimate.Price.Currency, ReservationID: reservation.Reservation.ID, State: "RESERVED", IdempotencyKey: r.IdempotencyKey, Fingerprint: r.Fingerprint, MaximumInputTokens: r.MaximumInputTokens, MaximumOutputTokens: r.MaximumOutputTokens, EstimatedCost: estimate.EstimatedCost, ReservedSale: estimate.MaximumSale, Rates: estimate.Price.Rates}
	var key, fingerprint any
	if r.IdempotencyKey != "" {
		key = r.IdempotencyKey
		fingerprint = r.Fingerprint[:]
	}
	_, err = tx.Exec(ctx, `INSERT INTO chat_request_charges(id,request_id,organization_id,project_id,api_key_id,protocol,operation,model,channel_id,price_id,maximum_input_tokens,maximum_output_tokens,currency,estimated_cost,reserved_sale,reservation_id,state,idempotency_key,request_fingerprint) VALUES($1,$2,$3,$4,$5,'openai','chat.completions',$6,$7,$8,$9,$10,$11,$12,$13,$14,'RESERVED',$15,$16)`, charge.ID, charge.RequestID, charge.OrganizationID, charge.ProjectID, charge.APIKeyID, charge.Model, charge.ChannelID, charge.PriceID, charge.MaximumInputTokens, charge.MaximumOutputTokens, charge.Currency, charge.EstimatedCost, charge.ReservedSale, charge.ReservationID, key, fingerprint)
	if err != nil {
		return Charge{}, err
	}
	if s.quota != nil {
		_, err = s.quota.ReserveInTx(ctx, tx, costquota.ReservationRequest{ChargeID: charge.ID, OrganizationID: charge.OrganizationID, ProjectID: charge.ProjectID, APIKeyID: charge.APIKeyID, Protocol: "openai", Operation: "chat.completions", Model: charge.Model, Currency: charge.Currency, Amount: charge.ReservedSale})
		if err != nil {
			return Charge{}, err
		}
	}
...
func (s *Service) CompleteUsage(ctx context.Context, id string, usage chatpricing.Usage, snapshot billing.ResponseSnapshot) (Charge, error) {
	...
	if _, err = s.wallet.CaptureInTx(ctx, tx, charge.ReservationID, amounts.Sale, "chat-capture:"+id); err != nil {
		return Charge{}, err
	}
	if s.quota != nil {
	...
	_, err = tx.Exec(ctx, `INSERT INTO chat_usage_evidence(charge_id,prompt_tokens,cached_input_tokens,completion_tokens,schema_version,body_sha256) VALUES($1,$2,$3,$4,'openai-chat-usage-v1',$5) ON CONFLICT(charge_id) DO NOTHING`, id, usage.PromptTokens, usage.CachedInputTokens, usage.CompletionTokens, digest[:])
	if err != nil {
		return Charge{}, err
	}
...
func (s *Service) Release(ctx context.Context, id string, snapshot billing.ResponseSnapshot) (Charge, error) {
	...
	if _, err = s.wallet.ReleaseInTx(ctx, tx, charge.ReservationID, "chat-release:"+id); err != nil {
		return Charge{}, err
	}
	if s.quota != nil {
...
const chargeSelect = `SELECT c.id,c.request_id,c.organization_id,c.project_id,c.api_key_id,c.model,c.channel_id,c.price_id,c.currency,c.reservation_id,c.state,c.idempotency_key,c.request_fingerprint,c.maximum_input_tokens,c.maximum_output_tokens,c.estimated_cost,c.reserved_sale,c.actual_cost,c.captured_sale,c.response_snapshot_version,c.response_status,c.response_headers,c.response_body,p.input_cost_per_million,p.input_sale_per_million,p.cached_input_cost_per_million,p.cached_input_sale_per_million,p.output_cost_per_million,p.output_sale_per_million FROM chat_request_charges c JOIN chat_token_prices p ON p.id=c.price_id`
...
func scan(row pgx.Row) (Charge, bool, error) {
	...
	var status *int
	err := row.Scan(&c.ID, &c.RequestID, &c.OrganizationID, &c.ProjectID, &c.APIKeyID, &c.Model, &c.ChannelID, &c.PriceID, &c.Currency, &c.ReservationID, &c.State, &key, &fp, &c.MaximumInputTokens, &c.MaximumOutputTokens, &c.EstimatedCost, &c.ReservedSale, &c.ActualCost, &c.CapturedSale, &c.SnapshotVersion, &status, &headers, &body, &c.Rates.InputCost, &c.Rates.InputSale, &c.Rates.CachedInputCost, &c.Rates.CachedInputSale, &c.Rates.OutputCost, &c.Rates.OutputSale)
...
func validBegin(r BeginRequest) bool {
	...
	return validPrefixed(r.OrganizationID, "org_") && validPrefixed(r.ProjectID, "project_") && validPrefixed(r.APIKeyID, "key_") && r.RequestID != "" && len(r.RequestID) <= 128 && r.Model != "" && len(r.Model) <= 200 && strings.TrimSpace(r.Model) == r.Model && validID(r.ChannelID, "channel_") && r.MaximumInputTokens > 0 && r.MaximumOutputTokens > 0 && ((r.IdempotencyKey == "" && !has) || (idempotency.Valid(r.IdempotencyKey) && has))
}
func sameRequest(c Charge, r BeginRequest) bool {
	...
	return identity && c.OrganizationID == r.OrganizationID && c.ProjectID == r.ProjectID && c.APIKeyID == r.APIKeyID && c.Model == r.Model && c.ChannelID == r.ChannelID && c.MaximumInputTokens == r.MaximumInputTokens && c.MaximumOutputTokens == r.MaximumOutputTokens
}
...
func validReason(v string) bool {
	switch v {
	case "executor_timeout", "executor_connection_lost", "response_unavailable", "usage_invalid", "settlement_failed", "provider_panic":
		return true
	}
	return false
}
