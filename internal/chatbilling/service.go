...
func (s *Service) CompleteStreamUsage(ctx context.Context, id string, usage chatpricing.Usage, terminalDigest [32]byte) (Charge, error) {
	...
	if charge.DeliveryMode != "stream" || usage.PromptTokens > charge.MaximumInputTokens || usage.CompletionTokens > charge.MaximumOutputTokens {
		return Charge{}, ErrInvalid
	}
	...
	if charge.State == "CAPTURED" {
		if charge.StreamCompleted && charge.CapturedSale == amounts.Sale && charge.ActualCost != nil && *charge.ActualCost == amounts.Cost {
			return charge, nil
		}
		return Charge{}, ErrConflict
	}
...
func (s *Service) MarkStreamReconcilingUsage(ctx context.Context, id string, usage chatpricing.Usage, terminalDigest [32]byte) error {
	...
	_, err = tx.Exec(ctx, `INSERT INTO chat_charge_reconciliations(charge_id,reason,prompt_tokens,cached_input_tokens,completion_tokens,terminal_category,terminal_event_sha256) VALUES($1,'settlement_failed',$2,$3,$4,'complete',$5) ON CONFLICT(charge_id) DO NOTHING`, id, usage.PromptTokens, usage.CachedInputTokens, usage.CompletionTokens, terminalDigest[:])
	if err != nil {
		return err
	}
...
func sameRequest(c Charge, r BeginRequest) bool {
	...
	return identity && c.OrganizationID == r.OrganizationID && c.ProjectID == r.ProjectID && c.APIKeyID == r.APIKeyID && c.Model == r.Model && c.ChannelID == r.ChannelID && c.MaximumInputTokens == r.MaximumInputTokens && c.MaximumOutputTokens == r.MaximumOutputTokens && c.DeliveryMode == r.DeliveryMode
}
