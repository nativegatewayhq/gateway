...
func TestRunStoresOnlyDigestAndPrintsKeyOnce(t *testing.T) {
	...
	code := run([]string{"-name", "cli integration", "-requests-per-minute", "120", "-burst", "10", "-allow-model", "openai:image.generate:gpt-image-1", "-allow-model", "gemini:image.generate:gemini-image", "-allow-cidr", "192.0.2.8/24"}, &stdout, &stderr, func(key string) string {
	...
	if !strings.Contains(stderr.String(), "network access: allowlist (1 prefixes)") {
		t.Fatalf("network summary=%q", stderr.String())
	}
	...
	if err := pool.QueryRow(ctx, `SELECT count(*) FROM service_api_key_network_prefixes p JOIN service_api_keys k ON k.id=p.api_key_id WHERE k.key_digest=$1 AND p.prefix='192.0.2.0/24'::cidr`, digest[:]).Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != 1 {
		t.Fatalf("network prefix rows=%d", count)
	}
	var leaked bool
