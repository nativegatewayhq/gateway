...
| `GATEWAY_OPENAI_TRANSLATION_MODELS` | unset | Comma-separated logical OpenAI audio translation models; managed mode additionally requires an active immutable duration price |
...
| `GATEWAY_SPEECH_OUTPUT_STORAGE_MODE` | `disabled` | `managed` enables opt-in private Speech output persistence and authenticated delivery |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_ENDPOINT` | unset | Private S3-compatible HTTPS endpoint; loopback HTTP is local-development only |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_REGION` | unset | S3 signing region; use `auto` for R2 |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_BUCKET` | unset | Private generated-Speech bucket; no object URL is exposed |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_ACCESS_KEY_ID` | unset | Secret-manager supplied least-privilege object access key ID |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_SECRET_ACCESS_KEY` | unset | Secret-manager supplied object secret; never logged |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_SERVER_SIDE_ENCRYPTION` | unset | Optional `AES256` object encryption request |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_MAX_BYTES` | `268435456` | Maximum captured bytes per generated Speech result; maximum 512 MiB |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_MAX_CONCURRENT_CAPTURES` | `4` | Process-wide bounded private capture spools; maximum 64 |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_UPLOAD_TIMEOUT` | `2m` | Complete private object upload timeout |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_DOWNLOAD_TIMEOUT` | `2m` | Complete authenticated object-read timeout |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_RETENTION` | `168h` | Speech asset lifetime from one hour through 30 days |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_CLEANUP_INTERVAL` | `1m` | Expiry and deletion worker interval |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_CLEANUP_LEASE` | `2m` | Cleanup ownership lease; maximum 10 minutes |
| `GATEWAY_SPEECH_OUTPUT_STORAGE_TEMP_DIR` | system temp | Directory for permission-restricted bounded capture spools |
...
Inbound multipart data is validated and reconstructed through bounded memory/file spools before being sent only to the fixed OpenAI `/v1/audio/translations` origin with a replaced Provider credential. Provider errors, timeouts, resets, panics, client cancellation, and response-limit failures never cause redispatch. Audio, filename, prompt, translated text, service keys, and Provider credentials are not stored or logged.

In billing-required mode, publish an immutable duration price and verified maximum duration before enabling a model:
...
gateway-audio-price \
  ...
  -channel-id channel_00000000000000000000000000000001 \
  -model translation-public \
  -publication-key openai-whisper-translation-2026-08 \
  -effective-from 2026-08-01T00:00:00Z \
  -cost-per-minute 60000 \
  -sale-per-minute 72000 \
  -maximum-duration-milliseconds 1500000
```
...
## Managed Speech output delivery

Set `GATEWAY_SPEECH_OUTPUT_STORAGE_MODE=managed` to opt individual native Speech requests into durable private delivery. Requests without the extension remain byte-compatible streaming requests.

```bash
curl https://gateway.example/v1/audio/speech \
  -H 'Authorization: Bearer SERVICE_API_KEY' \
  -H 'Content-Type: application/json' \
  -H 'Idempotency-Key: speech-result-1' \
  -H 'X-Native-Gateway-Delivery: managed' \
  -D headers.txt \
  -o speech.mp3 \
  -d '{"model":"tts-1","voice":"alloy","input":"Hello"}'

curl https://gateway.example/v1/audio/speech/assets/speechasset_.../content \
  -H 'Authorization: Bearer SERVICE_API_KEY' \
  -H 'Range: bytes=0-65535'
```

The create response remains native audio and adds only `X-Native-Gateway-Speech-Asset`. Metadata, authenticated `GET`/`HEAD` single-range content, and logical deletion are available below `/v1/audio/speech/assets/{id}`. Authorization rechecks the organization, project, and API-key snapshot; bucket URLs, object keys, digests, input text, voices, generated bytes, and credentials are never exposed in metadata or telemetry. The local Compose MinIO `gateway-audio-inputs` bucket may also be used for Speech output development, while production should use a separately permissioned private bucket.
